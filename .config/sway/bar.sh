...
cpu=$(read c u n s i w x y z < /proc/stat; t1=$((u+n+s+i+w+x+y)); i1=$((i+w)); sleep 0.05; read c u n s i w x y z < /proc/stat; t2=$((u+n+s+i+w+x+y)); i2=$((i+w)); dt=$((t2-t1)); di=$((i2-i1)); usage=$(echo "scale=1; (100*($dt-$di)/$dt)" | bc); printf "%04.1f" "$usage")
